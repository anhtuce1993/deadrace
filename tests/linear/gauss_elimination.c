...
main(int argc, char** argv) {
  ...
  if(MyRank == 0) {
     ...
     if ((fp = fopen ("tests/linear/mdatgaus.inp", "r")) == NULL) {
	...
	Matrix_A[irow] = (double *) malloc(n_size * sizeof(double));
     ...
     if ((fp = fopen ("tests/linear/vdatgaus.inp", "r")) == NULL){
  ...
  for (neigh_proc = 0; neigh_proc < MyRank; neigh_proc++) {
     ...
     for(irow = 0; irow < NoofRows_Bloc; irow++){
        ...
        /* .... Buffer_Pivot[0] is used to determine the starting value of pivot in each row of the matrix, on each processor            */
     ...
     endIter(neigh_proc, MyRank);
  }
  endtime = MPI_Wtime();
  endFor(Numprocs, 0, MyRank);
  ...
  endtime = MPI_Wtime();
  endFor(neigh_proc, endtime - starttime, MyRank);
  ...
  if (MyRank == 0) {
     ...
     fprintf(fp, " ------------------------------------------- \n");
     fprintf(fp, "Results of Gaussian Elimination Method on processor %d: \n", MyRank);
     fprintf (fp, "\n");

     fprintf(fp,"Solution vector \n");
     fprintf(fp, "\n");
     for(irow = 0; irow < n_size; irow++)
        fprintf(fp, "%.3lf  ", Output[irow]);
     fprintf(fp, "\n --------------------------------------------------- \n");
     fclose(fp);
  }
