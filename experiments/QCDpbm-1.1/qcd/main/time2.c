...
static double wt_norm_square(int nflds)
{
   ...
   for (ib=0;ib<1;nmax*=2)
   {
      ...
      {
         for (i=0;i<nflds;i++)
            r=norm_square_dble(VOLUME,1,psd[i][0]);
      }
...
static double wt_mulc_spinor_add(int nflds)
{
   ...
   {  
      ...
      {
         for (i=0;i<nflds;i+=2)
            mulc_spinor_add_dble(VOLUME,psd[i][0],psd[i+1][0],z);
