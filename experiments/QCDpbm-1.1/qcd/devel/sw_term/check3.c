...
int main(int argc,char *argv[])
{
   ...
   for (n=0;n<4;n++)
   {
      ...
      random_sd(VOLUME,ps0,1.0);
      apply_sw_dble(VOLUME,swd,ps0,ps1);
      mul_swd(lat.m0,lat.csw,ps0,ps2);
      mulg5_dble(VOLUME,ps2);

      z.re=-1.0;
      z.im=0.0;
      mulc_spinor_add_dble(VOLUME,ps2,ps1,z);
      d=norm_square_dble(VOLUME,1,ps2)/norm_square_dble(VOLUME,1,ps0);
