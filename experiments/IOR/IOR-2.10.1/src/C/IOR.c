    ...
    if (numTasks > 1) {
        if (rank == 0) {
            ...
            for (i = 0; i < numTasks-1; i++) {
                /*if (check == 0 && i == (numTasks - 2)) {
                    MPI_CHECK(MPI_Recv(hostname, MAX_STR, MPI_CHAR, 1,
...
TestIoSys(IOR_param_t *test)
{
    ...
    if (test->writeVal == NULL) ERR("out of memory");
    test->readVal = (double *)malloc(test->repetitions * sizeof(double));
    if (test->readVal == NULL) ERR("out of memory");
    for (rep = 0; rep < test->repetitions; rep++) {
